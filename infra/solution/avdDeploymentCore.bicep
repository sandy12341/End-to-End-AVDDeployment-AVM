...
var portalEffectiveBrownfieldDetectedDesktopAssignmentCount = length(brownfieldDetectedScopedApplicationGroupAssignments) > 0
...
var portalEffectiveBrownfieldDetectedRemoteAppAssignmentCount = length(brownfieldDetectedScopedApplicationGroupAssignments) > 0
...
var portalEffectiveBrownfieldDetectedDirectUserAssignmentCount = length(brownfieldDetectedScopedApplicationGroupAssignments) > 0
...
var portalEffectiveBrownfieldDetectedGroupAssignmentCount = length(brownfieldDetectedScopedApplicationGroupAssignments) > 0
...
var effectiveBrownfieldDetectedDesktopAssignmentCount = portalEffectiveBrownfieldDetectedDesktopAssignmentCount
...
var operationalSummaryFindings = concat(
  ...
  isDay2GenerateOperationalSummary && effectiveHostPoolType == 'Pooled' && brownfieldDetectedMaxSessionLimit <= 0 ? [
    {
      code: 'MAX_SESSION_LIMIT_NOT_CONFIGURED'
      severity: 'High'
      category: 'Capacity'
      bestPractice: 'Configure a valid max session limit on pooled host pools so load balancing and autoscale capacity calculations are meaningful.'
      observedState: 'The selected pooled host pool does not report a positive max session limit.'
      impact: 'Capacity planning, user-distribution expectations, and autoscale behavior can become unreliable.'
      recommendedAction: 'Set a positive max session limit on the host pool before relying on scaling and capacity assumptions.'
      canBeMitigatedBySolutionAction: false
      recommendedActionName: 'ManualFollowUp'
    }
  ] : [],
  isDay2GenerateOperationalSummary && effectiveHostPoolType == 'Personal' && !brownfieldDetectedStartVmOnConnect ? [
    {
      code: 'START_VM_ON_CONNECT_DISABLED'
      severity: 'Medium'
      category: 'Capacity'
      bestPractice: 'Use Start VM on Connect for personal host pools unless there is a deliberate alternative power-management design.'
      observedState: 'The selected personal host pool does not report Start VM on Connect as enabled.'
      impact: 'Users can see slower first-launch behavior or unavailable personal desktops if machines are powered down.'
      recommendedAction: 'Review whether Start VM on Connect should be enabled for this personal host pool.'
      canBeMitigatedBySolutionAction: false
      recommendedActionName: 'ManualFollowUp'
    }
  ] : [],
  isDay2GenerateOperationalSummary && brownfieldDetectedValidationEnvironment ? [
    {
      code: 'VALIDATION_ENVIRONMENT_ENABLED'
      severity: 'Informational'
      category: 'Governance'
      bestPractice: 'Keep validation-environment host pools clearly identified and separated from production support expectations.'
      observedState: 'The selected host pool reports validationEnvironment as enabled.'
      impact: 'Operational expectations, change control, and service-level assumptions may differ from standard production pools.'
      recommendedAction: 'Confirm that validation-environment status is intentional and reflected in operational ownership and support notes.'
      canBeMitigatedBySolutionAction: false
      recommendedActionName: 'ManualFollowUp'
    }
  ] : [],
  ...
  isDay2GenerateOperationalSummary && brownfieldDetectedSessionHostAmaCoverageEvaluated && length(brownfieldDetectedSessionHostVmNames) > 0 && length(brownfieldDetectedSessionHostsWithAma) < length(brownfieldDetectedSessionHostVmNames) ? [
    {
      ...
      severity: 'Medium'
      category: 'Monitoring'
      bestPractice: 'Install Azure Monitor Agent on every session host when guest monitoring posture is expected to be complete.'
      observedState: 'One or more discovered session hosts do not appear to have the Azure Monitor Agent extension in the selected host pool resource group.'
      impact: 'Guest telemetry coverage can be incomplete, reducing visibility into performance and OS-level issues.'
      recommendedAction: 'Run AlignMonitoringPosture with full monitoring posture, then verify Azure Monitor Agent coverage for all session hosts.'
      canBeMitigatedBySolutionAction: true
      recommendedActionName: 'AlignMonitoringPosture'
    }
  ] : [],
  isDay2GenerateOperationalSummary && brownfieldDetectedSessionHostDcrAssociationCoverageEvaluated && length(brownfieldDetectedSessionHostVmNames) > 0 && length(brownfieldDetectedSessionHostsWithDcrAssociation) < length(brownfieldDetectedSessionHostVmNames) ? [
    {
      ...
      severity: 'Medium'
      category: 'Monitoring'
      bestPractice: 'Associate every monitored session host with a data collection rule so guest telemetry reaches the workspace consistently.'
      observedState: 'One or more discovered session hosts do not appear to have a data collection rule association in the selected host pool resource group.'
      impact: 'AMA may be installed without complete telemetry routing, leaving guest-monitoring data gaps.'
      recommendedAction: 'Run AlignMonitoringPosture with full monitoring posture, then verify DCR association coverage for all session hosts.'
      canBeMitigatedBySolutionAction: true
      recommendedActionName: 'AlignMonitoringPosture'
    }
  ] : [],
  ...
  isDay2GenerateOperationalSummary && effectiveHostPoolType == 'Pooled' && length(brownfieldDetectedScalingPlanNames) > 0 && brownfieldDetectedSubscriptionPowerOnOffContributorAssignmentCount == 0 ? [
    {
      code: 'AUTOSCALE_POWER_ROLE_MISSING'
      severity: 'High'
      category: 'Resilience'
      bestPractice: 'Assign Desktop Virtualization Power On Off Contributor at subscription scope for Azure Virtual Desktop autoscale readiness.'
      observedState: 'No subscription-scope Desktop Virtualization Power On Off Contributor assignment was detected in the current subscription.'
      impact: 'Scaling plans can exist but autoscale power-management actions may not work correctly at runtime.'
      recommendedAction: 'Assign Desktop Virtualization Power On Off Contributor to the Azure Virtual Desktop service principal at subscription scope.'
      canBeMitigatedBySolutionAction: false
      recommendedActionName: 'ManualFollowUp'
    }
  ] : [],
  isDay2GenerateOperationalSummary && brownfieldDetectedScalingPlanDiagnosticsCoverageEvaluated && effectiveHostPoolType == 'Pooled' && length(brownfieldDetectedScalingPlanNames) > 0 && length(brownfieldDetectedScalingPlansWithDiagnostics) < length(brownfieldDetectedScalingPlanNames) ? [
    {
      ...
      severity: 'Medium'
      category: 'Monitoring'
      bestPractice: 'Enable Autoscale diagnostic settings on each scaling plan so autoscale operations and failures can be reviewed.'
      observedState: 'One or more detected scaling plans do not appear to have diagnostic settings in the selected host pool resource group.'
      impact: 'Autoscale execution issues can be harder to investigate because scaling-plan diagnostic logs are missing.'
      recommendedAction: 'Enable diagnostic settings on the scaling plan and route Autoscale logs to your monitoring destination.'
      canBeMitigatedBySolutionAction: false
      recommendedActionName: 'ManualFollowUp'
    }
  ] : []
...
var operationalSummaryObject = isDay2GenerateOperationalSummary
  ? {
      ...
      targetSummary: {
        ...
        startVmOnConnect: brownfieldDetectedStartVmOnConnect
        validationEnvironment: brownfieldDetectedValidationEnvironment
      }
      relatedResources: {
      ...
      resiliencePosture: {
        ...
        scalingPlanState: effectiveHostPoolType == 'Pooled'
          ...
          : 'NotApplicable'
        autoscaleReadiness: {
          powerOnOffContributorAssignmentCount: brownfieldDetectedSubscriptionPowerOnOffContributorAssignmentCount
          powerRoleState: effectiveHostPoolType == 'Pooled'
      ...
      discoveryCoverage: {
        assessed: [
          ...
          'Host pool max session limit and Start VM on Connect posture'
          ...
          'Session host AMA and DCR-association coverage inferred from resource-group resource discovery'
          'Scaling plan attachments'
          'Scaling plan diagnostics coverage inferred from resource-group resource discovery'
          'Subscription-scope Desktop Virtualization Power On Off Contributor assignment detection'
        ...
        notAssessed: [
          ...
          'Cross-resource-group guest monitoring coverage'
          'End-to-end autoscale runtime success'
        ]
      }
    }
  : {
...
var operationalSummaryStatusColor = operationalSummaryOverallStatus == 'HighRisk'
  ? '#8b1e3f'
  : (operationalSummaryOverallStatus == 'NeedsAttention'
    ? '#9a3412'
    : (operationalSummaryOverallStatus == 'Monitor'
      ? '#854d0e'
      : '#166534'))
var operationalSummaryFindingsTableRows = isDay2GenerateOperationalSummary
  ? (length(operationalSummaryFindings) > 0
    ? join(map(operationalSummaryFindings, (finding) => '<tr><td>${finding.severity}</td><td>${finding.category}</td><td>${finding.code}</td><td>${finding.observedState}</td><td>${finding.recommendedAction}</td></tr>'), '')
    : '<tr><td colspan="5">No findings were generated for the selected host pool.</td></tr>')
  : '<tr><td colspan="5">Operational summary was not requested.</td></tr>'
var operationalSummaryTopRecommendationsHtml = isDay2GenerateOperationalSummary && length(operationalSummaryTopRecommendations) > 0
  ? '<ul>${join(map(operationalSummaryTopRecommendations, (recommendation) => '<li><strong>${recommendation.priority}:</strong> ${recommendation.action}</li>'), '')}</ul>'
  : '<p>No remediation recommendations were generated.</p>'
var operationalSummaryHtml = isDay2GenerateOperationalSummary
  ? join([
      '<!doctype html>'
      '<html lang="en">'
      '<head>'
      '  <meta charset="utf-8"/>'
      '  <title>AVD Operational Summary</title>'
      '  <style>'
      '    body{font-family:Segoe UI,Arial,sans-serif;margin:24px;background:#f7f7f5;color:#1f2937;}'
      '    .card{background:#ffffff;border:1px solid #d4d4d8;border-radius:12px;padding:20px;margin-bottom:20px;box-shadow:0 2px 6px rgba(15,23,42,.06);}'
      '    .status{display:inline-block;padding:8px 14px;border-radius:999px;font-weight:600;color:#ffffff;background:${operationalSummaryStatusColor};}'
      '    table{border-collapse:collapse;width:100%;margin-top:12px;}'
      '    th,td{border:1px solid #d4d4d8;padding:10px 12px;text-align:left;vertical-align:top;}'
      ...
      '<body>'
      '  <div class="card">'
      '    <h1>Azure Virtual Desktop Operational Summary</h1>'
      '    <p><strong>Host pool:</strong> ${hostPoolName}</p>'
      '    <p><strong>Resource group:</strong> ${effectiveExistingHostPoolResourceGroupName}</p>'
      '    <p><strong>Generated:</strong> ${deploymentInstanceSeed}</p>'
      '    <p><span class="status">${operationalSummaryOverallStatus}</span></p>'
      '    <p>${operationalSummaryObject.executiveSummary.summaryText}</p>'
      '  </div>'
      '  <div class="card">'
      '    <h2>Summary</h2>'
      '    <table>'
      ...
      '      </thead>'
      '      <tbody>'
      '        <tr><td>Critical findings</td><td>${string(operationalSummaryCriticalCount)}</td></tr>'
      '        <tr><td>High findings</td><td>${string(operationalSummaryHighCount)}</td></tr>'
      '        <tr><td>Medium findings</td><td>${string(operationalSummaryMediumCount)}</td></tr>'
      '        <tr><td>Low findings</td><td>${string(operationalSummaryLowCount)}</td></tr>'
      '        <tr><td>Informational findings</td><td>${string(operationalSummaryInformationalCount)}</td></tr>'
      ...
      '        <tr><td>Validation environment</td><td>${brownfieldDetectedValidationEnvironment ? 'Enabled' : 'Disabled'}</td></tr>'
      ...
      '    ${operationalSummaryTopRecommendationsHtml}'
      '  </div>'
      '  <div class="card">'
      '    <h2>Findings</h2>'
      '    <table>'
      '      <thead>'
      '        <tr><th>Severity</th><th>Category</th><th>Code</th><th>Observed State</th><th>Recommended Action</th></tr>'
      '      </thead>'
      '      <tbody>'
      '        ${operationalSummaryFindingsTableRows}'
      '      </tbody>'
      '    </table>'
      '  </div>'
      '</body>'
  ...
  : '<!doctype html><html lang="en"><body><p>Operational summary was not requested.</p></body></html>'
var operationalSummaryHtmlDataUri = 'data:text/html;base64,${base64(operationalSummaryHtml)}'
